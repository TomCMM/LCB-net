##################################################################################
#	DESCRIPTION
#		Class "CleanDataLCB" -> Read data LCB observations, clean the possible error and write a new file
...
#			'H': 'T_St,Bat mV,S_10cm,S_20cm,S_30cm,S_40cm,S_60cm,S_100cm,,\r\n'
#		}
#		try
...
#
	...
	def clear(self):
		...
		for idx,line in enumerate(content_clear[self.NbLineHeader::]):
			if len(line.split(',')) != self.nbcol() or line[0:1].isdigit() ==False :
				print('-> Deleting the line ',idx+self.NbLineHeader," :",content_clear[self.NbLineHeader+idx])
...
############### ########### ########### ####### 
#====== User Input 
InPath='/home/thomas/PhD/obs-lcb/LCBData/obs/Extrema14260/Extrema14260'
OutPath='/home/thomas/PhD/obs-lcb/LCBData/obs/Extrema14260/Clean'
...
for i in Files:
	data=CleanDataLCB(os.path.dirname(i)+"/",os.path.basename(i))
	print("Writing file "+os.path.basename(i)+os.path.dirname(i) )
